...
// --- Simplified Servo Control Function (Updated with robust logic and clamping) ---
void updateServoPosition() {
    Serial.println("--- Entering updateServoPosition (Simplified and Robust) ---");
    time_t currentTime = ntpClient.getEpochTime();
    
    // --- Find the closest past tide and closest future tide ---
    ...
    // Use the actual time difference as the range for the map function.
    // This makes the mapping dynamic and more accurate for tides that are
    // not exactly 6 hours apart. The `MAX_TIDE_CYCLE_SECONDS` constant
    // is a flawed concept for a linear map.
    ...
    if (prevTideType == "H" && nextTideType == "L") {
        ...
        // Clamp to ensure it doesn't go below the low tide angle
    ...
    // A final constrain to the overall range
