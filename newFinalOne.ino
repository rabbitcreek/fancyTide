...
// --- Time and NOAA Data Functions (Updated to use start/end dates) ---
...
void updateServoPosition() {
    ...
    // Use a small value for currentTime to handle the NTP failure case
    // This is not an ideal solution but works as a stop-gap. A better solution
    // is to pass the current time as an argument.
    if(currentTime == 0) {
        Serial.println("NTP time is 0. Using last successful time from preferences if available.");
        // This is a bit of a hack but ensures the calculation doesn't go completely haywire
        // The time will be off, but the logic will at least try to use the last known good state
        currentTime = lastHighTideTime > lastLowTideTime ? lastHighTideTime : lastLowTideTime;
        if(currentTime == 0) {
            Serial.println("No valid last known time. Cannot update servo. Aborting.");
    ...
    if (lastHighTideTime > 0 && (lastLowTideTime == 0 || lastHighTideTime > lastLowTideTime)) {
    ...
    const long MAX_TIDE_CYCLE_SECONDS = 6 * 3600;
    
    int newAngle = SERVO_LOW_TIDE_ANGLE;
    
    if (prevTideType == "H" && nextTideType == "L") {
        newAngle = map(secondsIntoCycle, 0, MAX_TIDE_CYCLE_SECONDS, SERVO_HIGH_TIDE_ANGLE_1, SERVO_LOW_TIDE_ANGLE);
        ...
        newAngle = map(secondsIntoCycle, 0, MAX_TIDE_CYCLE_SECONDS, SERVO_LOW_TIDE_ANGLE, SERVO_HIGH_TIDE_ANGLE_2);
...
void deepSleep() {
  ...
  if (currentTime == 0) {
    ...
    esp_sleep_enable_timer_wakeup(600ULL * uS_TO_S_FACTOR); // Sleep for 10 minutes instead of 1 hour
...
// --- Arduino Setup and Loop (Updated with robust NTP check) ---
...
void setup() {
  ...
  bool ntpSuccess = getTimeFromNTP();

  // The rest of the setup logic now depends on the success of the NTP update.
  if (ntpSuccess) {
    time_t currentTime = ntpClient.getEpochTime();
    struct tm *ti = localtime(&currentTime);
    ...
    // If NTP failed, do not attempt to download new data.
    // The `lastDownloadDay` variable is not updated, so the next boot will still
    // trigger a new download if a new day has passed.
