...
const char* PREF_UTC_OFFSET = "utc_offset"; // New NVS key for UTC offset

// WiFiManager and NVS Objects
WiFiManager wm;
Preferences preferences;
WiFiUDP ntpUDP;
NTPClient ntpClient(ntpUDP, "pool.ntp.org", -8 * 3600); // Default to AKDT offset, will be updated later

// Global Configuration Variables
char wifi_ssid[60];
char noaa_station_id[10];
int lastDownloadDay = 0;
long utcOffset = -8; // New variable for UTC offset in hours
...
WiFiManagerParameter custom_utc_offset("utcoffset", "UTC Offset (e.g., -8 for AKDT)", "-8", 3); // New parameter


// --- NVS Helper Functions (Updated to handle UTC offset) ---
...
void saveUtcOffset(long offset) {
  preferences.begin(PREF_NAMESPACE, false);
  preferences.putLong(PREF_UTC_OFFSET, offset);
  preferences.end();
  Serial.printf("UTC Offset %ld saved to NVS.\n", offset);
}
...
void loadConfiguration() {
  ...
  utcOffset = preferences.getLong(PREF_UTC_OFFSET, -8); // Load the UTC offset, default to -8
  preferences.end();
  ...
  Serial.print("Loaded UTC Offset: "); Serial.println(utcOffset);
}


// --- WiFiManager Configuration (Updated) ---
...
bool connectToWiFi() {
  ...
  wm.addParameter(&custom_utc_offset); // Add new UTC offset parameter
  ...
  if (wm.autoConnect("TideClockSetupAP", "password")) {
    ...
    // New logic to handle UTC offset from the portal
    long newUtcOffset = atol(custom_utc_offset.getValue());
    if (newUtcOffset != utcOffset) {
      utcOffset = newUtcOffset;
      saveUtcOffset(utcOffset);
      Serial.printf("UTC Offset updated to %ld and saved via portal.\n", utcOffset);
    }
...
// --- Time and NOAA Data Functions (Unchanged) ---

bool getTimeFromNTP() {
  Serial.println("Getting time from NTP...");
  // Initialize NTP client with the loaded or new UTC offset
  ntpClient.setTimeOffset(utcOffset * 3600);
  ntpClient.begin();
